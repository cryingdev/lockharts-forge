...
export const TavernListView: React.FC<TavernListViewProps> = ({ hired, visitors, onInvite, onSelect, inviteCost }) => {
    return (
            ...
            <div className="relative z-10 h-full p-3 md:p-5 overflow-y-auto custom-scrollbar flex flex-col gap-6 pb-20">
                <div className="flex justify-end items-end border-b border-stone-800 pb-3 shrink-0">
                    <div className="flex items-center gap-2">
                        <SfxButton onClick={onInvite} className="bg-stone-900 border border-stone-700 px-4 py-1.5 rounded-lg text-stone-300 flex items-center gap-2 hover:border-amber-500 transition-all shadow-md active:scale-95 group">
                            <PlusCircle className="w-3.5 h-3.5 text-amber-500 group-hover:rotate-90 transition-transform duration-300" />
                            <span className="text-[10px] font-black uppercase tracking-tight">Invite ({inviteCost}G)</span>
                        </SfxButton>
                    </div>
                </div>
